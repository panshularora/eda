...
| | |
|---|---|
| Reviews **enumerated** in the window | **{{reviews_enumerated}}** |
| Reviews **kept** (quota sample) | **{{reviews_kept}}** ({{sampling_fraction}} of the frame) |
| Apps reaching the full window | **{{apps_complete_window}} / {{apps_total}}** |
| Corpus after normalisation | **{{normalised_rows}}** rows, {{window_span_days}} days ({{window_start}} → {{window_end}}) |
| Delay-related | **{{n_delay}}** ({{delay_share}}) |
| Brands · brands in the balanced panel | {{brands_total}} · **{{brands_in_panel}}** |
...
| | {{case1_label}} | {{case2_label}} |
|---|---|---|
| mean rating | {{case1_rating_before}} → {{case1_rating_during}} | {{case2_rating_before}} → {{case2_rating_during}} |
| effect on reviewers | *d* = {{case1_d}} | *d* = {{case2_d}} |
| delay-related share | {{case1_delay_before}} → **{{case1_delay_during}}** | {{case2_delay_before}} → **{{case2_delay_during}}** |
| outage language | {{case1_outage_before}} → {{case1_outage_during}} | {{case2_outage_before}} → **{{case2_outage_during}}** |
...
Of {{n_case_events}} brand-days where a rating fell more than 1.5 SD below that
...
the window and keeps a **quota sample of at most {{quota_per_brand_day}} per
...
| this collection (census + quota sample) | **{{r_all_brands}}** | {{r_panel}} |
...
On the balanced panel the series now averages {{panel_mean_daily}} delay-related
reactions a day at a coefficient of variation of {{cv_panel}}.
...
|---|---:|---:|
| PELT change points | 9 | {{n_shifts}} |
| "engagement spikes" | 27 | {{n_spikes}} ({{n_spikes_volume}} volume, {{n_spikes_endorsement}} endorsement) |
| shifts surviving Benjamini–Hochberg over the whole scan | **0** | **{{scan_survivors}}** of {{scan_tests}} tests |
...
to finding {{scan_survivors}} shifts that survive correction across
{{scan_tests}} tests.
...
| | |
|---|---|
| Independent labels | **{{transfer_n}}** star ratings |
| Three-class accuracy | {{transfer_accuracy}} (macro-F1 {{transfer_macro_f1}}, κ {{transfer_kappa}}) |
| **Polarity only** (Neutral dropped) | **{{polarity_accuracy}}** on {{polarity_n}} rows, κ {{polarity_kappa}} |
| At confidence ≥ 0.70 | accuracy {{gate70_accuracy}}, covering {{gate70_coverage}} of rows |

A model trained on 2015-era tweets reaches **{{polarity_accuracy}} agreement on
positive-vs-negative** in 2026 app-store reviews — from a completely different
domain, register and decade.

**Neutral is where it breaks.** Recall {{neutral_recall}}, precision
{{neutral_precision}}, and it emits **{{neutral_predicted}}** Neutral
predictions where there are only **{{neutral_true}}** three-star reviews —
over-emitting the class by {{neutral_over_emission}}×. Because
...
Positive. τ = {{recal_tau}} is fitted on half the star-labelled rows; every
number below is measured on the other {{recal_n_test}}, which the fitting never
saw.

| held-out | accuracy | Cohen's κ | Neutral predictions |
|---|---:|---:|---:|
| argmax (as shipped) | {{argmax_accuracy}} | {{argmax_kappa}} | {{argmax_neutral_predicted}} |
| recalibrated | **{{recal_accuracy}}** | **{{recal_kappa}}** | {{recal_neutral_predicted}} |
| *truth* | — | — | *{{recal_true_neutral}}* |

**+{{recal_accuracy_gain}} accuracy and +{{recal_kappa_gain}} κ from a single
...
the learned topic head agrees with it on **{{topic_rule_agreement}} of rows**.
...
| after the audit | **{{n_delay}}** | **1.40** | **4.3%** |
...
discarded **{{dropped_duplicate_text}} rows non-randomly**: short, common texts
...
found nothing; on this one it finds **{{scan_survivors}}** across
{{scan_tests}} tests.
...
|---|---|---|
| `company_replied` | {{reply_rate}} of delay rows | whether the operator answered — operator-controlled, fast-moving, needs no model to read |
| `stated_delay_hours` | {{duration_share}} of delay rows | median {{duration_median_hours}} h — the severity axis polarity cannot see |
...
endpoints are read. The Play census made **{{play_pages}} page requests**;
